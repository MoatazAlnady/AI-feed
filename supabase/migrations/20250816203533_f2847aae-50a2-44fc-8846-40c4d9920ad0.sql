...
BEGIN
  ...
  -- Handle status column conversion to enum with proper default handling
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name = 'tool_edit_requests' 
    ...
    ALTER TABLE public.tool_edit_requests ALTER COLUMN status TYPE edit_status USING status::edit_status;
...
-- Updated functions for handling edit requests (using user_id instead of requester_id)
CREATE OR REPLACE FUNCTION public.approve_tool_edit_request_v2(
  request_id UUID,
...
CREATE OR REPLACE FUNCTION public.reject_tool_edit_request_v2(
  request_id UUID,
