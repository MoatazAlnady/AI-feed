...
CREATE POLICY IF NOT EXISTS "Users can view conversations they participate in" ON public.conversations
  FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "Users can add participants to conversations" ON public.conversation_participants
...
CREATE POLICY IF NOT EXISTS "Users can view messages in their conversations" ON public.conversation_messages
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.conversation_participants cp
            WHERE cp.conversation_id = conversation_messages.conversation_id AND cp.user_id = auth.uid())
  );

CREATE POLICY IF NOT EXISTS "Users can send messages to their conversations" ON public.conversation_messages
  FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.conversation_participants cp
            WHERE cp.conversation_id = conversation_messages.conversation_id AND cp.user_id = auth.uid())
...
CREATE POLICY IF NOT EXISTS "Users can view mentions that involve them" ON public.mentions
  FOR SELECT USING (mentioned_user_id = auth.uid() OR mentioner_user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Users can create mentions" ON public.mentions
...
-- Update trigger for conversation_messages  
...
DROP TRIGGER IF EXISTS update_conversation_messages_updated_at ON public.conversation_messages;
