...
import { MessageCircle, X, Bot, Send, Users, Loader2 } from 'lucide-react';
...
const EmployerChatDock: React.FC<EmployerChatDockProps> = ({ className = '', onTalentSearch }) => {
  ...
  const handleSendAiMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const aiResponse = {
        id: Date.now() + 1,
        text: data?.response || `I'm searching for talents based on: "${newMessage.text}". Check the talent search filters for matching profiles!`,
        sender: 'ai' as const
      };
              ...
              <div className="h-80 overflow-y-auto">
                {chatsLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-sm p-4 text-center">
                    ...
                    <p className="text-xs">Connect with candidates!</p>
                  </div>
                ) : (
                  chats.map((chat) => (
                    <div
